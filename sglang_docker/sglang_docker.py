#!/usr/bin/env python3
"""Manage sglang runtime via Docker across DGX Spark nodes.

Similar to sglang_runtime but runs inside a Docker container instead of a
local Python venv. Same subcommand surface (launch/stop/logs/scan/benchmark/
measure) plus `pull` for pre-pulling images.
"""
...
def _build_docker_launch_command(
    ...
    """Build the docker run command for sglang launch."""
    container = _container_name(preset_name) if preset_name else "sglang-server"
    ...
    extra_sglang = " ".join(shlex.quote(arg) for arg in sglang_args)
...
def launch(args: argparse.Namespace) -> int:
    ...
    if not launch_cmd:
        ...
        if args.mode == "cluster":
            # Container uses host networking, so dist-init-addr works the same
            host_count = len(args.hosts) if args.hosts else 2
            dist_args = f"--dist-init-addr {dist_addr} --nnodes {host_count}"
    ...
    if args.mode == "solo":
        if args.host:
            ...
            cname = _container_name(preset_name) if preset_name else "sglang-server"
            result = run_remote(
                args.host,
    ...
    for idx, host in enumerate(hosts):
        node_dist = f"--dist-init-addr {dist_addr} --nnodes {len(hosts)} --node-rank {idx}"
        node_cmd = _build_docker_launch_command(
            ...
            sglang_args=extra_sglang_args,
            env=env,
            preset_name=f"{preset_name}-node{idx}" if preset_name else f"sglang-node{idx}",
            log_dir=log_dir,
            dist_args=node_dist,
        )
        cname = _container_name(f"{preset_name}-node{idx}") if preset_name else f"sglang-node{idx}"
    ...
    if args.mode == "solo":
        ...
        if args.hosts:
            ...
            if not hosts:
                ...
                return 2
        if preset_name:
            targets = [(h, f"{preset_name}-node{idx}") for idx, h in enumerate(hosts)]
        else:
            targets = [(h, f"sglang-node{idx}") for idx, h in enumerate(hosts)]
...
def logs(args: argparse.Namespace) -> int:
    ...
    if preset_name:
        cname = f"{preset_name}-node{rank}"
    else:
        cname = f"sglang-node{rank}"
    host = hosts[rank]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DGX Spark sglang runtime via Docker")
    ...
    p_launch = sub.add_parser("launch", help="Launch sglang via Docker")
    ...
    p_launch.add_argument(
        "--log-dir",
        default=None,
        help="Log directory (default: ~/sglang-docker-logs; env: LOG_DIR)",
    )
    ...
    p_launch.add_argument(
        "--sglang-args",
        default="",
        help="Extra arguments appended to sglang.launch_server",
    )
    ...
    p_stop = sub.add_parser("stop", help="Stop sglang Docker container(s)")
